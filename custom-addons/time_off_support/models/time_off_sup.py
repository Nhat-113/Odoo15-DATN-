...
class ExportWizard(models.TransientModel):
    ...
    def get_time_off_type_unpaid_ids_for_companies(self, company_ids):
        time_off_type_unpaid_ids = [0]
        companies = self.env['res.company'].browse(company_ids)
        
        for company in companies:
            if company.time_off_type_unpaid_id:
                time_off_type_unpaid_ids.append(company.time_off_type_unpaid_id.id)
                
        return time_off_type_unpaid_ids
