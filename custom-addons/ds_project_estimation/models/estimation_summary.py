...
class EstimationSummaryCostRate(models.Model):
    ...
    currency_id = fields.Many2one('estimation.currency', store=True, )  # compute='_compute_currency_id'
    ...
    vnd_day = fields.Float(string="Unit (VND/Day)", store=True, compute='_compute_yen_month', default=0.00)
    ...
    def _compute_yen_month(self):
        for item in self:
            ...
            if cost_rate:
                ...
                value_exchange = self.env['estimation.exchange.rate'].search(
                    [('currency_id', '=', currency_id.id)]).value
                if currency_id.id == 1:
