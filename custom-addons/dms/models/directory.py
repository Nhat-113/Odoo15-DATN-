...
class DmsDirectory(models.Model):
    ...
    def _compute_count_total_directories(self):
        for record in self:
            count = (
                self.search_count(
                    [("id", "child_of", record.id)]) if record.id else 0
            )
    ...
    def _check_directory_storage(self):
        for record in self:
            if record.is_root_directory and not record.storage_id:
                raise ValidationError(
                    _("A root directory has to have a storage."))
    ...
    def _alias_get_creation_values(self):
        ...
        values["alias_model_id"] = self.env["ir.model"].sudo()._get(
            "dms.directory").id
        if self.id:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("parent_id", False):
                ...
                vals["storage_id"] = self._convert_to_write(
                    data).get("storage_id")
        ...
        if vals.get("storage_id"):
            for item in self:
                if item.storage_id.id != vals["storage_id"]:
                    raise UserError(
                        _("It is not possible to change the storage."))
