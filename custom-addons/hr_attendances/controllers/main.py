...
class HrAttendance(http.Controller):
    ...
    @http.route("/api/employee/attendance", auth="api_key", type="json", methods=["POST"], csrf= False)
    def new_attendance(self, **kw):
        ...
        try:
            ...
            if not kw.get('type'):
                return {"status": 400, "message":"Missing required parameter type"}
            ...
            formatted_time = valid_timezone(timezone, timestamp)
...
class HrEmployees(http.Controller):
    ...
    @http.route("/api/sync/employee", type='http', auth="api_key", methods=["GET"])
    def employee_list(self, **kw):
        ...
        try:
            ...
            total_records = request.env['hr.employee'].sudo().search_count([])
            last_pages = -(-total_records // limit)
            employees = (request.env['hr.employee'].sudo().search([], offset=records_to_skip, limit=limit))
            get_id_employee = [employee['id'] for employee in employees]
            employee_result = { emp.id: emp.image_1920 for emp in request.env['hr.employee'].search([('id', 'in', get_id_employee)])}
                ...
                "employees": [
                    {
                        "id": employee.id,
                        "email": employee.work_email,
                        # "image": image_url_getter('hr.employee', employee.id),
                        "image": employee_result[employee.id],
                    }
                    for employee in employees
