            ...
            CREATE OR REPLACE VIEW %s AS (
				...
				FROM (
						...
						WHEN (EXTRACT(MONTH FROM BRM.START_DATE_MONTH) IS NULL) THEN 
							CASE
								...
								WHEN (EXTRACT(YEAR FROM (SELECT(MIN( HC2.DATE_START)) FROM HR_CONTRACT AS HC2 WHERE HC2.EMPLOYEE_ID=EMP.ID)) = EXTRACT(YEAR FROM NOW()) ) THEN 
									...
									END	 
							END
						WHEN ((EXTRACT(YEAR FROM (SELECT(MIN( HC2.DATE_START)) FROM HR_CONTRACT AS HC2 WHERE HC2.EMPLOYEE_ID=EMP.ID)) < EXTRACT(YEAR FROM BRM.START_DATE_MONTH)) AND (PP.ID = (SELECT PCR2.PROJECT_ID FROM PUBLIC.PLANNING_CALENDAR_RESOURCE PCR2 WHERE BRM.BOOKING_ID=PCR2.ID))) THEN
							CASE 
					...
					(
						...
						FROM HR_CONTRACT 
						WHERE EMPLOYEE_ID=EMP.ID AND (STATE = 'open' OR STATE = 'close') 
					),
					(
						SELECT
							STRING_AGG(
